...
export async function fetchJurisdictions() {
  await delay(500);
  const jurisdictions = fakeJurisdictionsData.jurisdictions.map((j) => ({
    id: j.id,
    name: j.name,
  }));
...
function findJurisdiction(jurisdictionId) {
  return findJurisdictionRecursive(
    fakeJurisdictionsData.jurisdictions,
    jurisdictionId
  );
}
...
export async function fetchSubJurisdictions(jurisdictionId) {
  ...
  return (jurisdiction.subJurisdictions || []).map((j) => ({
    id: j.id,
    name: j.name,
  }));
}
